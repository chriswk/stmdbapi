...
object Dependencies {
  ...
  val typesafeConfig = "com.typesafe" % "config" % "1.0.2"
	def dispatchDep(artifactName: String) = "net.databinder.dispatch" %% s"dispatch-${artifactName}" % dispatchVersion

	def slf4jDep(artifactName: String) = "org.slf4j" % s"slf4j-${artifactName}" % slf4jVersion

	val dispatchDependencies = Seq(dispatchDep("core"), dispatchDep("lift-json"))
  val typesafeDependencies = Seq(typesafeConfig)
	val loggingDependencies = Seq(slf4jDep("api"), slf4jDep("simple"))
	val testDependencies = Seq(specs2 % "test")
	val apiDependencies = dispatchDependencies ++ typesafeDependencies ++ loggingDependencies ++ testDependencies
}